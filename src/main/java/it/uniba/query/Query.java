	...
	public Query(final Arguments args) throws ArgumentException {
		...
		if (args.getType() == null) {
			...
			query += "SELECT distinct owner_user_id\\r\\n FROM\r\n" + "((SELECT distinct owner_user_id\r\n"
					+ " FROM `bigquery-public-data.stackoverflow.posts_answers`\r\n" + " WHERE "
					+ date + " and owner_user_id is not null)\r\n" + "UNION ALL\r\n"
					+ "(SELECT distinct owner_user_id\r\n"
					+ " FROM `bigquery-public-data.stackoverflow.posts_questions`\r\n" + " WHERE "
					+ date + " and owner_user_id is not null))\r\n order by owner_user_id\r\n";
		} else if (args.getType().equals("answer") && args.getTaglike() != null && args.getEdge() != true
			...
			query += "SELECT distinct owner_user_id\\r\\n FROM (SELECT distinct owner_user_id" + " FROM\r\n"
					...
					+ " FROM `bigquery-public-data.stackoverflow.posts_questions`\r\n" + " WHERE "
					+ date + " and REGEXP_CONTAINS(tags, r\"" + args.getTaglike() + "\") \r\n"
					+ " and owner_user_id is not null))\r\n order by owner_user_id\r\n";
		} else if (args.getType().equals("question") && args.getTaglike() != null && args.getEdge() != true
				&& args.getWeight() != true && args.getUser() == 0) {
			query += "SELECT distinct owner_user_id\\r\\n FROM `bigquery-public-data.stackoverflow.posts_questions`\r\n"
					+ " WHERE " + date + " and REGEXP_CONTAINS(tags, r\"" + args.getTaglike()
					+ "\") \r\n" + " and owner_user_id is not null\r\n order by owner_user_id\r\n";
		} else if (args.getType().equals("answer") && args.getTaglike() == null && args.getEdge() == true
			...
			query += "SELECT `from`,`to`, count(*) as weight\r\n" + "FROM(\r\n"
				...
				&& args.getWeight() == true && date.equals("") && args.getUser() != 0) {
